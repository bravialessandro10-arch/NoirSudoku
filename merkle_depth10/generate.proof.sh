...
VK_START=$(date +%s%N)
bb write_vk -b ./target/merkle_depth10.json -o ./target --oracle_hash keccak
VK_END=$(date +%s%N)
...
bb prove -b ./target/merkle_depth10.json \
    -w ./target/merkle_depth10.gz \
    -o ./target \
    --oracle_hash keccak
PROOF_END=$(date +%s%N)
...
WITNESS_SIZE=$(stat -f%z ./target/merkle_depth10.gz 2>/dev/null || stat -c%s ./target/merkle_depth10.gz)
...
─────────────────────────────────────────────────────────────
  ...
  - Depth 10 (attuale):        1,024 elementi
  - Depth 12:                  4,096 elementi
