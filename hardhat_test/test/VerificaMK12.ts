...
describe("HonkVerifier On-Chain Verification", function () {
  it("Verifica proof zk on-chain", async function () {
    ...
    const TranscriptLibFactory = await ethers.getContractFactory("contracts/VerifierMK12.sol:ZKTranscriptLib");
    ...
    const VerifierFactory = await ethers.getContractFactory("contracts/VerifierMK12.sol:HonkVerifier", {
      libraries: {
    ...
    const proofPath = path.join(__dirname, "../../merkle_depth12/target/proof_hex.txt");  
    const proof = fs.readFileSync(proofPath, "utf8").trim();

    // Carica publicInputs array da file (parse JSON)
    const publicInputsPath = path.join(__dirname, "../../merkle_depth12/target/public_inputs_array.txt");
